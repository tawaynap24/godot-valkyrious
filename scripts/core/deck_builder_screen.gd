...
func _on_card_gui_input(event: InputEvent, card_id: String) -> void:
	# Discard emulated mouse events generated by emulate_mouse_from_touch
...
func _on_deck_slot_input(event: InputEvent, slot_index: int) -> void:
	if slot_index >= _working_deck.size():
		return
	# Filter emulated mouse from touch; fire only on real touch/mouse press
	var is_click: bool = (
		(event is InputEventScreenTouch and event.pressed)
		or (event is InputEventMouseButton and event.device != -1
